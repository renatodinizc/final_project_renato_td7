...
describe Proposal do
  context 'validation on' do
    it 'the proposal description attribute successfully' do
      ...
      expect(proposal.errors.full_messages_for(:proposal_description)).to include
    ...
    it 'the hourly wage attribute successfully' do
      ...
      expect(proposal.errors.full_messages_for(:hourly_wage)).to include
      'Valor/hora não pode ficar em branco'
    end
    it 'the weekly hours attribute successfully' do
      ...
      'Horas semanais não pode ficar em branco'
    end
    it 'the expected conclusion attribute successfully' do
      ...
      expect(proposal.errors.full_messages_for(:expected_conclusion)).to include
      'Conclusão esperada não pode ficar em branco'
    end
    ...
    it 'uniqueness per freelancer successfull' do
      ...
      desafios = Project.create!(title: 'Plataforma de desafios de programação',
                                 ...
                                 contractor: peter,
                                 freelancer_expertise: webdev)
      Proposal.create!(proposal_description: 'Sou ex-aluno da Campus Code e quero contribuir com a plataforma',
      ...
      proposal2 = Proposal.new(proposal_description: 'Sou ex-aluno da Campus Code e quero contribuir com a plataforma2',
                               hourly_wage: 37, weekly_hours: 13,
                               expected_conclusion: 1.month.from_now,
                               project: desafios, freelancer: spongebob, contractor: peter)

      proposal2.valid?

      expect(proposal2.errors.full_messages_for(:freelancer_id)).to include
      'Freelancer não pode enviar mais de uma proposta simultânea para mesmo projeto'
    end
    ...
    it 'uniqueness does not take into account archived proposals successfull' do
      ...
      proposal2 = Proposal.new(proposal_description: 'Sou ex-aluno da Campus Code e quero contribuir com a plataforma2',
                               hourly_wage: 37, weekly_hours: 13,
                               expected_conclusion: 56.weeks.from_now,
                               project: desafios, freelancer: spongebob, contractor: peter)

      proposal2.valid?

      expect(proposal2.errors.full_messages_for(:freelancer_id)).not_to include
      'Freelancer não pode enviar mais de uma proposta simultânea para mesmo projeto'
    end
  end
  ...
  context 'custom validation on wage' do
    it 'higher border successfully' do
      ...
      expect(proposal.errors.full_messages_for(:hourly_wage)).to include
      'Valor/hora não pode exceder o máximo preço por hora do projeto'
    end
  ...
  context 'custom validation on expected conclusion' do
    it 'cannot be in the past' do
      ...
      expect(proposal.errors.full_messages_for(:expected_conclusion)).to include
      'Conclusão esperada não pode ser no passado'
    end
    ...
    it 'cannot be earlier than projects proposal deadline' do
      ...
      expect(proposal.errors.full_messages_for(:expected_conclusion)).to include
      'Conclusão esperada não pode ser antes do prazo de submissão final do projeto'
    end
    it 'cannot be on the same day of project proposal deadline' do
      ...
      expect(proposal.errors.full_messages_for(:expected_conclusion)).to include
      'Conclusão esperada não pode ser no mesmo dia do prazo de submissão final do projeto'
    end
  end
end
